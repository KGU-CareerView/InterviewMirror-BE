...
public class FeedbackAggregationService {
  ...
  private FeedbackResult buildResult(String sessionId, List<FeedbackResponse> responses) {
    ...
    return FeedbackResult.builder()
        ...
        .startedAt(
            toDateTime(
                responses.stream().mapToLong(FeedbackResponse::getTimestamp).min().orElse(0L)))
        .endedAt(endedAt)
        .build();
  }
  ...
  private int countLabel(List<FeedbackResponse> responses, String label) {
    return (int)
        responses.stream().filter(response -> label.equalsIgnoreCase(response.getLabel())).count();
  }

  private int countFaceDetected(List<FeedbackResponse> responses) {
    return (int) responses.stream().filter(FeedbackResponse::isFaceDetected).count();
  }
  ...
  private double average(
      List<FeedbackResponse> responses, Function<FeedbackResponse, Float> extractor) {
    return responses.stream().map(extractor).mapToDouble(Float::doubleValue).average().orElse(0.0);
  }
