...
- **Java 17 이상**
- **Gradle 9.4.1 이상** (또는 제공된 Gradle Wrapper 사용)
- **MySQL 8.4 이상**
- **Redis 8.0 이상** (프로덕션 환경)
...
| 명령어 | 설명 |
|--------|------|
| `make` / `make up` | 로컬 빌드 후 전체 스택 시작 |
| `make build` | 로컬 JAR 빌드만 (컨테이너 실행 X) |
| `make start` | 기존 이미지로 컨테이너 시작 (재빌드 X) |
| `make stop` | 컨테이너 중지 (볼륨/이미지 유지) |
| `make down` | 컨테이너 삭제 (볼륨/이미지 유지) |
| `make clean` | 컨테이너 + 볼륨 삭제 (DB 초기화) |
| `make fclean` | 컨테이너 + 볼륨 + 이미지 전부 삭제 |
| `make re` | 완전 초기화 후 재빌드 |
| `make deploy` | backend만 로컬 빌드 후 재배포 (MySQL/Redis 유지) |
| `make logs` | 전체 서비스 로그 확인 |
| `make logs-backend` | 특정 서비스 로그 확인 |
| `make ps` | 컨테이너 상태 확인 |
| `make test` | 로컬 테스트 실행 |
...
```bash
# backend만 빠르게 재배포 (MySQL, Redis 유지)
make deploy
...
SPRING_PROFILE_ACTIVE=dev ./gradlew bootRun
...
| `main` | 프로덕션 배포 | - | - |
| `dev` | 개발 통합 | `main` | `main` |
| `feature/*` | 기능 개발 | `dev` | `dev` |
| `bugfix/*` | 버그 수정 | `dev` | `dev` |
| `hotfix/*` | 긴급 수정 | `main` | `main`, `dev` |
| `release/*` | 배포 준비 | `dev` | `main`, `dev` |
...
- GitHub에서 PR 생성
- Base: `dev`, Compare: `feature/IM-123-user-auth`
- 템플릿에 따라 PR 설명 작성
- 코드 리뷰 요청 (최소 1명)
