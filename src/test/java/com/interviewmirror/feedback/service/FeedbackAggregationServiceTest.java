...
class FeedbackAggregationServiceTest {
    ...
    @Mock
    private FeedbackBufferRepository feedbackBufferRepository;
    ...
    void aggregateAndSave() {
        List<FeedbackResponse> responses = List.of(
                ...
                response("session-1", 3000L, "stable_confident", 0.9f, true, "Keep steady eye contact.")
        );
        ...
        when(feedbackRepository.save(any(FeedbackResult.class))).thenAnswer(invocation -> invocation.getArgument(0));
    ...
    void aggregateAndSaveIdempotently() {
        FeedbackResult existing = FeedbackResult.builder()
                .id(1L)
                .sessionId("session-1")
                .totalFrames(1)
