...
// Security 설정을 무시하고 컨트롤러만 가볍게 띄우기 위한 제외 옵션 추가
...
class SessionControllerTest {
    ...
    void updateSessionStatusApiTest() throws Exception {
        ...
        mockMvc.perform(patch(BASE_URL + "/100/status")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestJson))
    ...
    void getPresignedUrlApiTest() throws Exception {
        ...
        mockMvc.perform(post(BASE_URL + "/100/presigned")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestJson))
                .andExpect(status().isOk())
    ...
    void saveMediaUrlApiTest() throws Exception {
        ...
        mockMvc.perform(post(BASE_URL + "/100/save/video")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestJson))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.Result").value("SUCCESS"))
    ...
    void submitAnswerApiTest() throws Exception {
        ...
        mockMvc.perform(post(BASE_URL + "/100/answer")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestJson))
    ...
    void analyzeEmotionApiTest() throws Exception {
        ...
        mockMvc.perform(post(BASE_URL + "/100/emotion")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestJson))
                .andExpect(status().isOk())
